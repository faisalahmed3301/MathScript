...
with tempfile.TemporaryDirectory() as directory:
    ...
    def surface(expr, bounds='-5 5 -5 5 -5 5', samples=24):
        ...
        points = json.loads(html.split('const points=')[1].split(';\nconst names=')[0])
        assert str(len(points))+' sampled surface points' in output
    ...
    assert all(abs(x*x+y*y+z*z-16)<1e-8 for x,y,z in points)
    ...
    assert points and all(abs(z-x*x+y*y)<1e-8 for x,y,z in points)
    points, _ = surface('x=0.137')
    assert points and all(abs(x-.137)<1e-10 for x,y,z in points)
    assert min(y for x,y,z in points)==-5 and max(z for x,y,z in points)==5
    points, _ = surface('x^2+y^2=9')
    assert points and all(abs(x*x+y*y-9)<1e-8 for x,y,z in points)
    ...
    assert all(abs(z-round(z))<1e-7 for x,y,z in points)
    points, _ = surface('sin(x)+cos(y)')
    assert points and all(abs(z-math.sin(x)-math.cos(y))<1e-8 for x,y,z in points)
    points, _ = surface('sqrt(x-2)+y=z')
    assert points and all(x>=2 and abs(math.sqrt(x-2)+y-z)<1e-8 for x,y,z in points)
    points, _ = surface('z=log(x)+y')
    assert points and all(x>0 and abs(z-math.log(x)-y)<1e-7 for x,y,z in points)
    points, _ = surface('1/(z-0.137)+0*x+0*y=0')
    assert not points, 'a pole is not a zero'
    points, output = surface('x^2+y^2+z^2=-1')
    assert not points
    lines=output.splitlines()
    start=next(i for i,line in enumerate(lines) if '120 x 45 canvas' in line)+1
    rows=[line[2:] for line in lines[start:start+45]]
    assert all(set(row)<=set(' .XYZ') for row in rows), '3D axes should have only dense dots and labels'
    for axis in range(3):
        for step in range(201):
            value=-5+step/20
            p=[0.,0.,0.]; p[axis]=value/10
            u=(p[0]+p[2])/math.sqrt(2)
            v=p[1]*math.sqrt(2/3)+(p[0]-p[2])/math.sqrt(6)
            col=math.floor(59.5+u*50+.5); row=math.floor(22-v*25+.5)
            assert rows[row][col]=='.', f'missing 3D axis {axis} unit {value}' 
    points, _ = surface('(z-0.137)^4+0*x+0*y=0')
    assert points and all(abs(z-.137)<1e-7 for x,y,z in points), 'tangent sheet lost'
    points, _ = surface('z=x*(x-1)*(x+1)*(x-2)+y')
    assert points and all(abs(z-x*(x-1)*(x+1)*(x-2)-y)<1e-7 for x,y,z in points), 'quartic misclassified as quadratic'
    points, _ = surface('(x^2+y^2+z^2+5)^2=36*(x^2+y^2)')
    assert points and all(abs((x*x+y*y+z*z+5)**2-36*(x*x+y*y))<1e-6 for x,y,z in points)
    assert any(z>1 for x,y,z in points) and any(z<-1 for x,y,z in points)
    points, _ = surface('a^2+b^2+c^2=16')
    assert points and all(abs(a*a+b*b+c*c-16)<1e-8 for a,b,c in points)
    points, _ = surface('(x-20)^2+(y-20)^2+(z-20)^2=4', '17 23 17 23 17 23')
    assert points and all(abs((x-20)**2+(y-20)**2+(z-20)**2-4)<1e-7 for x,y,z in points)
    ...
    before = set((cwd/'output').iterdir())
    out=run('/graph3d\nvccw\nx+y+z+w=0\nz=bad(x)\nz=sqrt(x,y)\nexit\n/graph2d\nx+y+z=0\n/graph\nexit', cwd)
    assert out.count('Semantic Error')==4, out
    assert set((cwd/'output').iterdir())==before, 'invalid equations exported a graph'
    ...
    assert set((cwd/'output').iterdir())==before
